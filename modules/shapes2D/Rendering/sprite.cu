...
void Sprite::draw(Framebuffer framebuffer, float drawx, float drawy, float scale) {
...
uint32_t Sprite::sample(float u, float v) {
    int sx = x + width * u;
    int sy = y + height * v;
...
SpriteSheet::SpriteSheet(uint32_t *data) : data(data) {
    moneyDisplay.x = 0;
    moneyDisplay.y = 0;
