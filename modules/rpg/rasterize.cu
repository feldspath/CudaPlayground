...
void drawMap(Map *map, uint64_t *framebuffer, int2 mappos, int pixelScale) {
    ...
    processRange(px_width * px_height, [&](int localPixelIndex) {
        ...
        if (maptile_x == mouse_tx)
            if (maptile_y == mouse_ty)
                if (maptile_local_x == 0 || maptile_local_x == 15 || maptile_local_y == 0 ||
                    maptile_local_y == 15) {
                    color = 0x00ffffff;
                }
...
extern "C" __global__ void kernel(const Uniforms _uniforms, unsigned int *buffer,
                                  ...
                                  uint32_t *textureData, uint32_t *img_ascii_16) {
    auto grid = cg::this_grid();
    ...
    processRange(0, 1024 * 1024, [&](int localPixelIndex) {
        ...
        float2 uv = {float(x) / 1024.0f, float(y) / 1024.0f};
