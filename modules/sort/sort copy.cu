...
void radix_sort(
	...
	if(grid.thread_rank() == 0){
		...
		int last = min(100, numElements) - 1;
		for(int i = 0; i <= last; i++){

			int64_t value = intermediateStorage[i];

			printf("%4u, ", value);

			if(i < last){
				int64_t next = intermediateStorage[i + 1];

				if(next < value){
					printf("\x1B[31m!!WRONG!!\x1B[97m");
				}
			}
