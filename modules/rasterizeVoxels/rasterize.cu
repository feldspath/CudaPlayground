...
// From A Ray-Box Intersection Algorithm and Efficient Dynamic Voxel Rendering (https://jcgt.org/published/0007/03/04/paper.pdf)
...
#define TEST(U, V, W) (d.U >= 0.0) &&                                          \
						  fabs(ray.origin.V + ray.dir.V * d.U) < box.radius && \
																 fabs(ray.origin.W + ray.dir.W * d.U) < box.radius
	int3 test = int3{TEST(x, y, z), TEST(y, z, x), TEST(z, x, y)};
	sgn = test.x ? float3{sgn.x, 0.0f, 0.0f} : (test.y ? float3{0.0f, sgn.y, 0.0f} : float3{0.0f, 0.0f, test.z ? sgn.z : 0.0f});
#undef TEST
		...
		for (int loop_i = 0; loop_i < loop_max; loop_i++)
		{
			...
			for (int fragOffset = 0; fragOffset < numFragments; fragOffset += block.num_threads())
			{
				...
				auto pixelDirection_C = [&](int2 pixelCoords)
				{
					...
					float4 ndc = {
						px.x / uniforms.width * 2.0f - 1.0f,
						px.y / uniforms.height * 2.0f - 1.0f,
						1.0,
						1.0};
				...
				float3 camPos_W = float3{
					uniforms.invview.rows[0].w,
					uniforms.invview.rows[1].w,
				...
				float3 fragColor = color * min(max(-dot(hitInfo_W.normal, lightDir_W), 0.0f) + 0.2f, 1.0f);
