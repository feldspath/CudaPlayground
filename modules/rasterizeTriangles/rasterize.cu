                ...
                if (s >= 0.0)
                    if (t >= 0.0)
                        if (s + t <= 1.0) {
                            ...
                            float2 uv = {v * v0_uv.x + s * v1_uv.x + t * v2_uv.x,
                            ...
                            } else if (colorMode == COLORMODE_TIME ||
                                       colorMode == COLORMODE_TIME_NORMALIZED) {
                                // TIME
                                uint64_t nanotime;
...
extern "C" __global__ void kernel(const Uniforms _uniforms, unsigned int *buffer,
                                  cudaSurfaceObject_t gl_colorbuffer, uint32_t numTriangles,
                                  float3 *positions, float2 *uvs, uint32_t *colors,
    ...
    { // draw the triangle mesh that was passed to this kernel
        ...
        for (float ox : {-3.0f, -1.5f, 0.0f, 1.5f, 3.0f})
            for (float oy : {-3.0f, -1.5f, 0.0f, 1.5f, 3.0f}) {
                ...
                mat4 wiggle =
                    mat4::rotate(cos(5.0f * uniforms.time) * 0.1f, {0.0f, 1.0f, 0.0f}).transpose();
                mat4 wiggle_yaw =
                    mat4::rotate(cos(5.0f * uniforms.time) * 0.1f, {0.0f, 0.0f, 1.0f}).transpose();
